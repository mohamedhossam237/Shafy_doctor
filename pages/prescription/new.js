...
export default function NewPrescriptionPage() {
  ...
  // Track if title was auto-generated (to allow editing after generation)
  ...
  // Set current date/time automatically on component mount
  React.useEffect(() => {
    if (!form.dateStr) {
      setForm((prev) => ({ ...prev, dateStr: getCurrentDateTimeString() }));
    }
  ...
  // Generate title with AI at the end (before save)
  const generateTitleAtEnd = React.useCallback(async () => {
    // Check if main clinical fields are filled (at least one significant field)
    const hasMainData = 
      form.diagnosis?.trim() || 
      form.findings?.trim() || 
    ...
    if (!hasMainData) return { titleAr: form.titleAr, titleEn: form.titleEn };

    // Only generate titles that are empty
    const needsArabic = !form.titleAr?.trim();
    const needsEnglish = !form.titleEn?.trim();
    
    if (!needsArabic && !needsEnglish) {
      return { titleAr: form.titleAr, titleEn: form.titleEn };
    }
    ...
    try {
      ...
      const generateTitle = async (lang, isAr) => {
        ...
        try {
          ...
          return title || (isAr ? form.titleAr : form.titleEn) || '';
        } catch (err) {
          console.error(`Error generating ${lang} title:`, err);
          return isAr ? form.titleAr : form.titleEn;
        }
      };

      // Generate both titles in parallel
      const [titleAr, titleEn] = await Promise.all([
        needsArabic ? generateTitle('Arabic', true) : Promise.resolve(form.titleAr || ''),
        needsEnglish ? generateTitle('English', false) : Promise.resolve(form.titleEn || ''),
      ]);

      return { titleAr: titleAr || form.titleAr || '', titleEn: titleEn || form.titleEn || '' };
    } catch (err) {
  ...
  // Title generation is now done at the end (before save), not automatically during typing
  ...
  const handleSave = async () => {
    ...
    // Check if at least one field has data (excluding title, as it will be generated)
    const hasAnyData = 
      form.diagnosis?.trim() ||
      form.findings?.trim() ||
      form.procedures?.trim() ||
    ...
    setSubmitting(true);
    try {
      // Generate titles at the end (before save) if they are empty
      const currentTitleAr = form.titleAr?.trim() || '';
      const currentTitleEn = form.titleEn?.trim() || '';
      const generatedTitles = await generateTitleAtEnd();
      
      // Use generated titles if original was empty, otherwise keep original
      let finalTitleAr = currentTitleAr || generatedTitles.titleAr || '';
      let finalTitleEn = currentTitleEn || generatedTitles.titleEn || '';
      
      // Mark as auto-generated if they were generated (only if they were empty before)
      if (!currentTitleAr && generatedTitles.titleAr) {
        setTitleAutoGenerated((prev) => ({ ...prev, ar: true }));
      }
      if (!currentTitleEn && generatedTitles.titleEn) {
      ...
      // Prepare all data with final titles
      const allData = {
        ...form,
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  ...
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      // Once user starts editing, allow free editing (don't mark as manually edited, just allow)
                    }}
                    disabled={!form.titleAr?.trim() && !titleAutoGenerated.ar}
                    helperText={
                      !form.titleAr?.trim() && !titleAutoGenerated.ar
                        ? t('Title will be generated automatically when you save the report', 'سيتم توليد العنوان تلقائياً عند حفظ التقرير')
                        : titleAutoGenerated.ar
                        ? t('Auto-generated - you can edit', 'مولّد تلقائياً - يمكنك التعديل')
                        : form.titleAr?.trim()
                        ? t('You can edit the title', 'يمكنك تعديل العنوان')
                        : ' '
                    }
                    InputProps={{
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  <TextField
                    ...
                    disabled={!form.titleEn?.trim() && !titleAutoGenerated.en}
                    helperText={
                      !form.titleEn?.trim() && !titleAutoGenerated.en
                        ? t('Title will be generated automatically when you save the report', 'سيتم توليد العنوان تلقائياً عند حفظ التقرير')
