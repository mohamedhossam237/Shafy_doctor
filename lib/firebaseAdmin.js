...
function init() {
  ...
  if (!admin.apps.length) {
    ...
    if (!credential) {
      // Initialization will fail later if anything tries to use auth/db.
      // Prefer throwing here if you want to fail-fast:
      // throw new Error('firebase-admin credentials not available');
      // For now, init with no credential (works on GCP with ADC).
      admin.initializeApp();
    } else {
